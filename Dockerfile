...
ARG GRAFANA_TGZ="grafana-latest.linux-amd64.tar.gz"
...
COPY ./grafana/run.sh $GF_PATHS_HOME/
COPY ./grafana/setup.sh $GF_PATHS_HOME/

RUN chown "grafana:$GF_GID_NAME" $GF_PATHS_HOME/run.sh
RUN chown "grafana:$GF_GID_NAME" $GF_PATHS_HOME/setup.sh
RUN chmod +x $GF_PATHS_HOME/run.sh
RUN chmod +x $GF_PATHS_HOME/setup.sh
...
RUN chown -R "grafana:$GF_GID_NAME" /etc/grafana
